...
@register_command(
    name="/citations",
    category="Information",
    short_desc="Inspect retrieval citation markers",
    long_desc="Inspect citation markers generated by search_chunks. "
    "Use turn mode for current-turn evidence or id mode for a specific marker.",
    usage="/citations [turn [N]|id <CITATION_ID>]",
    examples=[
        "/citations             Show citations from current turn",
        "/citations turn 20     Show up to 20 citations from current turn",
        "/citations id C-main-0001   Show one citation by id",
    ],
    related=["/rag-debug", "/tools"],
    accepts_args=True,
)
def cmd_citations(ctx: CommandContext, args: str) -> bool:
    """Inspect persisted citation markers from retrieval results."""
    from flavia.content.indexer.citation_log import read_recent_citation_entries

    raw = args.strip()
    parts = raw.split() if raw else []
    mode = parts[0].lower() if parts else "turn"

    if mode not in {"turn", "id"}:
        ctx.console.print("[red]Usage: /citations [turn [N]|id <CITATION_ID>][/red]")
        return True

    if mode == "id":
        if len(parts) != 2:
            ctx.console.print("[red]Usage: /citations id <CITATION_ID>[/red]")
            return True
        citation_id = parts[1].strip()
        entries = read_recent_citation_entries(
            ctx.settings.base_dir,
            limit=1,
            citation_id=citation_id,
        )
        if not entries:
            ctx.console.print(f"[yellow]Citation '{citation_id}' not found.[/yellow]")
            return True
        entry = entries[0]
        _print_citation_entry(ctx, entry, idx=1, total=1)
        return True

    # turn mode
    limit = 50
    if len(parts) > 2:
        ctx.console.print("[red]Usage: /citations turn [N][/red]")
        return True
    if len(parts) == 2:
        ...
        if limit <= 0 or limit > 500:
            ctx.console.print("[red]N must be between 1 and 500.[/red]")
            return True
    ...
    if not turn_id:
        ...
        ctx.console.print(
            "[dim]Send a new prompt first, then run /citations turn to inspect evidence markers.[/dim]"
        )
        return True
    ...
    if not entries:
        ctx.console.print(f"[yellow]No citations found for current turn ({turn_id}).[/yellow]")
        ctx.console.print(
            "[dim]Citations are created by search_chunks results. If none exist, the turn may not have retrieved evidence.[/dim]"
        )
        return True

    for idx, entry in enumerate(entries, start=1):
        _print_citation_entry(ctx, entry, idx=idx, total=len(entries))
        if idx != len(entries):
            ctx.console.print("")
    return True


def _print_citation_entry(ctx: CommandContext, entry: dict[str, Any], *, idx: int, total: int) -> None:
    """Render one citation entry consistently for /citations command."""
    citation_id = str(entry.get("citation_id") or "unknown")
    turn_id = str(entry.get("turn_id") or "")
    doc_name = str(entry.get("doc_name") or "unknown")
    ...
    line_parts: list[str] = []
    line_start = locator.get("line_start") if isinstance(locator, dict) else None
    line_end = locator.get("line_end") if isinstance(locator, dict) else None
    if line_start is not None and line_end is not None:
        line_parts.append(f"lines {line_start}-{line_end}")
    elif line_start is not None:
        line_parts.append(f"line {line_start}")
    elif line_end is not None:
        line_parts.append(f"line {line_end}")

    time_start = locator.get("time_start") if isinstance(locator, dict) else None
    time_end = locator.get("time_end") if isinstance(locator, dict) else None
    if time_start is not None and time_end is not None:
        line_parts.append(f"time {time_start}-{time_end}")
    elif time_start is not None:
        line_parts.append(f"time {time_start}")

    location = ", ".join(line_parts) if line_parts else "location unavailable"
    heading = " > ".join(str(h) for h in heading_path) if isinstance(heading_path, list) and heading_path else ""

    ctx.console.print(
        f"[bold]Citation {idx}/{total}[/bold] [cyan]{citation_id}[/cyan] [dim]{timestamp}[/dim]"
    )
    if turn_id:
        ctx.console.print(f"turn_id: {turn_id}")
    if query:
        ctx.console.print(f"query: {query}")
    ctx.console.print(f"source: {doc_name}")
    if modality:
        ctx.console.print(f"modality: {modality}")
    if heading:
        ctx.console.print(f"heading: {heading}")
    ctx.console.print(f"locator: {location}")
    if excerpt:
        ctx.console.print(f'excerpt: "{excerpt}"')
